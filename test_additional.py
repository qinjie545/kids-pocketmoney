...
# 使用测试数据库
TEST_DATABASE_PATH = os.path.join(os.path.dirname(__file__), 'database', 'test_cash_manager_additional.db')
        ...
        for i in range(5):
            ...
            cursor.execute(
                '''INSERT INTO transactions (user_id, type, amount, description, category, created_at)
                   VALUES (?, ?, ?, ?, ?, ?)''',
                (1, 'income', 10.0, f'测试收入{i}', '零花钱', transaction_date.isoformat())
            )
